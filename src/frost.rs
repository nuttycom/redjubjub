...
impl<S: SpendAuth> Zeroize for Secret<S> {
    fn zeroize(&mut self) {
        self.0 = S::Scalar::zero();
    }
}

impl<S: SpendAuth> Drop for Secret<S> {
    fn drop(&mut self) {
        self.zeroize();
    }
}
...
pub struct PublicKeyPackage<S: SpendAuth> {
    ...
    /// signing operation.
    pub(crate) signer_pubkeys: HashMap<u32, Public<S>>,
...
pub fn keygen_with_dealer<R: RngCore + CryptoRng, S: SpendAuth>(
    ...
    let mut signer_pubkeys: HashMap<u32, Public<S>> = HashMap::with_capacity(num_signers as usize);
...
// TODO finish drop
impl<S: SpendAuth> Drop for SigningNonces<S> {
    fn drop(&mut self) {}
}
...
impl<S: SpendAuth> SigningNonces<S> {
        ...
        R: CryptoRng + RngCore,
    {
        let mut bytes = [0; 64];
        rng.fill_bytes(&mut bytes);
        let hiding = S::Scalar::from_bytes_wide(&bytes);

        let mut bytes = [0; 64];
        rng.fill_bytes(&mut bytes);
        let binding = S::Scalar::from_bytes_wide(&bytes);
...
impl<S: SpendAuth> From<(u32, &SigningNonces<S>)> for SigningCommitments<S> {
    /// For SpendAuth signatures only, not Binding signatures, in RedJubjub/Zcash.
    fn from((index, nonces): (u32, &SigningNonces<S>)) -> Self {
        Self {
            index,
...
fn gen_rho_i<S: SpendAuth>(index: u32, signing_package: &SigningPackage<S>) -> S::Scalar {
...
fn gen_group_commitment<S: SpendAuth>(
    ...
    bindings: &HashMap<u32, S::Scalar>,
) -> Result<GroupCommitment<S>, &'static str> {
    let mut accumulator = S::Point::identity();
...
pub fn sign<S: SpendAuth>(
    signing_package: &SigningPackage<S>,
    participant_nonces: &SigningNonces<S>, // TODO this should probably consume the nonce
    share_package: &SharePackage<S>,
) -> Result<SignatureShare<S>, &'static str> {
    let mut bindings: HashMap<u32, S::Scalar> =
